...
const BASE_URL = 'https://www.objectivewire.com';
...
/**
 * Fix 4b: Strip "| objectivewire.com" and "| Object Wire" brand suffix variants
...
function fixBrandSuffixVariant(metaBlock: string, changes: string[]): string {
  return metaBlock.replace(
    ...
    (_match, prefix, quote, value) => {
      const cleaned = value
        .replace(/\s*\|\s*ObjectWire\.org\s*$/i, '')  // legacy: also handles old objectivewire.com brand suffix
