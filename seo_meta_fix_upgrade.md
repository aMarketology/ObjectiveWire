...
- `https://www.owire.org/robots.txt` → 122 lines (dynamic ✅, not the deleted static)
- `https://www.owire.org/sitemap.xml` → 700 URLs ✅
- `https://www.owire.org/news-sitemap.xml` → 37 URLs in 3-day window ✅ (M8 working)
...
- Homepage canonical = `https://www.owire.org`, count = 1 ✅
...
1. **Site-wide canonical override.** [app/layout.tsx](app/layout.tsx) hardcoded `<link rel="canonical" href={SITE_CONFIG.url} />` inside `<head>`. Every page emitted **two** canonical tags — its own self-canonical from `metadata.alternates.canonical`, plus a second one pointing to `https://www.owire.org`. Google's canonicalizer collapsed many article URLs into the homepage, suppressing them from search.
...
| URL | Expected |
|---|---|
| `https://www.owire.org/robots.txt` | 300+ line dynamic robots, AI bot rules present |
| `https://www.owire.org/sitemap.xml` | 200, ~690 URLs, no change |
| `https://www.owire.org/news-sitemap.xml` | 200, ≥30 URLs in 2-day window |
| `https://www.owire.org/entertainment/news/fortnite-moves-into-movies` | **Exactly one** `<link rel="canonical">` pointing at self |
| `https://www.owire.org/crypto/news/anchorage-usat-expands-to-celo-network` | Same |
| `https://www.owire.org/influencer/ari-kytsya` | Same |
