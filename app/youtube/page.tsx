...
export default async function Page() {
  ...
  const articles = all
    .filter((e) => {
      const slugMatch = e.slug.startsWith('/youtube/');
      const catMatch = ['YouTube', 'youtube', 'Entertainment'].some(
      ...
      <header className="border-b-2 border-black bg-white">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 py-10">
          ...
          <div className="flex items-start gap-4">
            <div className="h-12 w-1.5 bg-red-600 shrink-0 mt-1" />
            <div>
              ...
              <p className="mt-2 text-gray-600 text-base sm:text-lg max-w-2xl">
                Creators, channels, platform updates, and the creator economy.
              </p>
            </div>
          </div>
        </div>
              ...
              {articles.map((article) => (
                ...
                >
                  {article.imageUrl ? (
                    ...
                    <div className="w-full aspect-[16/9] bg-red-50 border-red-200 border-b flex items-center justify-center shrink-0">
                      <span className="text-red-600 text-3xl font-black opacity-30">OW</span>
                    </div>
                  )}
                  <div className="p-4 flex flex-col flex-1">
                    <span className="text-[9px] font-black px-2 py-0.5 tracking-[.15em] uppercase text-white bg-red-600 self-start mb-2">
                      {article.category}
                    </span>
