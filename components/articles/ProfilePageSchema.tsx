...
export interface ProfilePageSchemaProps {
  ...
  /** Meta description */
  description: string;
  /** Canonical page URL — must be https://www.objectivewire.com/... */
  pageUrl: string;
...
export function ProfilePageSchema({
  ...
  const socialUrls = sameAs.filter(
    (url) =>
      url.startsWith('http') &&
      !url.startsWith('https://www.objectivewire.com') &&
      !url.startsWith('mailto:'),
  );

  // Derive section from pageUrl so breadcrumb matches actual page path
  const urlSegments = pageUrl.replace('https://www.objectivewire.com', '').split('/').filter(Boolean);
  const sectionSlug = urlSegments[0] ?? 'creator';
  const sectionName = sectionSlug.charAt(0).toUpperCase() + sectionSlug.slice(1);
  const sectionUrl = `https://www.objectivewire.com/${sectionSlug}`;
  ...
  const schema = {
      ...
      ...(imageUrl && {
        'image': {
          ...
          'url': imageUrl.startsWith('http')
            ? imageUrl
            : `https://www.objectivewire.com${imageUrl}`,
          'width': 1200,
          'height': 675,
        },
      }),
    ...
    'publisher': {
      ...
      'logo': {
        '@type': 'ImageObject',
        'url': 'https://www.objectivewire.com/objectwire-logo.png',
        'width': 600,
        'height': 60,
      },
    },
