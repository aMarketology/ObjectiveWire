...
export default function sitemap(): MetadataRoute.Sitemap {
  const baseUrl = 'https://www.objectivewire.com';
...
export function SEOWrapper({ slug, children }: { slug: string; children: React.ReactNode }) {
  ...
  const articleSchema = {
    ...
    mainEntityOfPage: `https://www.objectivewire.com${entry.slug}`,
  };
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: slug.split('/').filter(Boolean).map((segment, i, arr) => ({
      ...
      item: `https://www.objectivewire.com/${arr.slice(0, i + 1).join('/')}`,
    })),
  };
...
async function pingGoogle() {
  // Ping sitemap
  await fetch(
    `https://www.google.com/ping?sitemap=https://www.objectivewire.com/sitemap.xml`
  );
